...
Router.prototype.dispatch = function (url, context, done, routeMatchIndex) {
  var routeMatchIndex = routeMatchIndex || 0;
  var self = this;

  assert(typeof url === 'string', "expected url string in router dispatch");

  var controller = this._currentController;
  var route = this.findRoute(url, routeMatchIndex);
  ...
  if (controller && route && prevRoute === route) {
    ...
    controller = this.createController(url, context, routeMatchIndex);
  }

  this._currentController = controller;

  var onRouterDispatchCompleted = function (err){
    if (err && !err instanceof RouteController.WrongRoute)
      throw err;
    else {
      // if exception exists and it was thrown because of currently used route is wrong
      //   we should redispatch router to call next matching route
      if (err && err instanceof RouteController.WrongRoute){
        self.dispatch(url, context, done, ++routeMatchIndex);
      }
      // ...otherwise, go back to the normal implementation of the dispatcher
      else if (!controller.isHandled()){
  ...
  controller.dispatch(self._stack, url, onRouterDispatchCompleted, routeMatchIndex);
