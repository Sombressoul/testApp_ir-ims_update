...
Router.prototype.dispatch = function (url, context, done, routeMatchIndex) {
  ...
  var controller = this.createController(url, context, routeMatchIndex);
  ...
  var onRouterDispatchCompleted = function (err){
    ...
    if (err) {
      ...
      // if exception was thrown because of currently used route is wrong
      //   we should stop current computations and redispatch router to call
      //   next matching route
      if ( err instanceof RouteController.WrongRoute ){
  ...
  controller.dispatch(this._stack, url, onRouterDispatchCompleted, routeMatchIndex);
};
