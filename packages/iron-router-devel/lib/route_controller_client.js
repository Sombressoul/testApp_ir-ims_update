...
RouteController.prototype._runRoute = function (route, url, done) {
  ...
  // create stack of 'onBeforeRun' hook and layout prerendering
  self._onBeforeRunExecuted = false;

  var onBeforeRunStack = new MiddlewareStack;

  var useLayoutPrerendering = typeof this.lookupOption('layoutPrerendering') !== 'undefined'
    ? !!this.lookupOption('layoutPrerendering')
    : true; // layout prerendering is "on" by default for backward compatibility

  var onBeforeRunHooks = !!this._collectHooks('onBeforeRun').length
    ? this._collectHooks('onBeforeRun')
    : function(req, res, next){ next(); }; // default

  var onBeforeRunExecuted = function (req, res, next) {
    self._onBeforeRunExecuted = true;
    next();
  };

  var layoutPrerendering = function(req, res, next){
    self.beginRendering(function onCompleteRenderingTransaction (usedRegions) {
      if (self.isStopped)
        return;
    ...
    self.layout(self.lookupOption('layoutTemplate'), {
      data: self.lookupOption('data')
    });

    next();
  };

  if (useLayoutPrerendering) {
    onBeforeRunStack.append(layoutPrerendering, onBeforeRunHooks, onBeforeRunExecuted, {where: 'client'});
  }
  else {
    onBeforeRunStack.append(onBeforeRunHooks, onBeforeRunExecuted, layoutPrerendering, {where: 'client'});
  }
  ...
  stack = stack.concat(onBeforeRunStack);
  ...
  Deps.afterFlush(function() {
    Iron.utils.warn(self._rendered || self.isStopped || !self._onBeforeRunExecuted,
      "Route dispatch never rendered. Did you forget to call this.next() in an onBeforeRun/onBeforeAction?");
  });
