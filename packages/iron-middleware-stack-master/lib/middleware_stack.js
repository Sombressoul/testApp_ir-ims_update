 ...
 */
MiddlewareStack.prototype.dispatch = function dispatch (url, context, done, matchIndex) {
  var self = this;
  var originalUrl = url;
  ...
  var next = Meteor.bindEnvironment(function boundNext (err) {
    ...
    // skip matched handler to execute handler with needed index if necessary
    if ( typeof matchIndex !== "undefined" && matchIndex > 0 ) {
      matchIndex--;
      return next();
    }
